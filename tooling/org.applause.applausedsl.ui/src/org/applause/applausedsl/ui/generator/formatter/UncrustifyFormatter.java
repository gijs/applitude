...
/**
 * Xpand Postprocessor for code formatting using the {@link http://uncrustify.sourceforge.net/ uncrustify} tool.
 * <p>
 * <h2>Configuration</h2>
 * 
 * <pre>
 ...
 * @see http://www.compuphase.com/pawn/pawn.htm
 */
public class UncrustifyFormatter implements PostProcessor {
	...
	private Pattern pattern_objc = Pattern.compile(".*\\.(m|h)$");

	/** This flag is used to produce log messages only once per instantiation, and not per invocation. */
	private boolean logonceflag;
	 ...
	 * @param unformatted Unformatted code
	 * @param args 
	...
	public CharSequence format (CharSequence unformatted, List<String> args) {
		...
		try {
			...
			if (logonceflag==false) {
				LOG.info("Uncrustify path: "+uncrustifyPath);
				logonceflag = true;
			}
			...
			if (rc!=0) {
				...
				if (rc!=0) {
					...
					if (LOG.isDebugEnabled()) LOG.debug("Execution of uncrustify was successful.");
				}
			}
		} catch (Exception re) {
	...
	protected List<String> initArgs (FileHandle fileHandle) throws IllegalArgumentException {
		...
		if (pattern_objc.matcher(filename).matches()) {
			args.add("OC");
		}
